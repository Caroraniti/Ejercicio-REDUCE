...
const sumatoria = (array) =>{
    ...
    const sumatoria = array.reduce((acc,elemento)=>{
      return acc + elemento
    },0)
    return sumatoria
    }
...
const productoria = (array) => {
  const productoria = array.reduce((acc, elemento) => {
    return acc * elemento;
  });
  return productoria;
};
...
/*const promedio = (notas) =>{
 ...
 Otra forma de resolucion! dividir cada elemento por la cantidad de notas y luego sumar eso a acc.*/

/*const notasDeTPs = [4, 7, 8, 5, 10];
const promedio = (notas) =>{
    const promedio = notas.reduce((acc,elemento,index,array)=>{
...
const librosDeJS = [
  ...
  const enlistar = (array) => {
    const lista = array.reduce((acc, elemento) => {
  ...
  const sumaDeEdades = (array) => {
    const suma = array.reduce((acc, elemento) => {
  ...
  // codear acá la solución del ejercicio
  /*const sumaDeLosDoblesPositivos = (datos) => {
    const total = datos.reduce((acc, dato, index, array) => {
      const positivos = array.filter((dato) => {
        return dato > 0;
      });
      const datosMultiplicados = positivos.map((dato)=>{
        const datoMultiplicado = dato * 2
        return acc + datoMultiplicado
      })
  ...
  const sumaDeLosDoblesPositivos = (datos) => {
        ...
        const positivos = datos.filter((dato) => {
          return dato > 0;
        });
        const datosMultiplicados = positivos.map((dato)=>{
         return dato * 2
        ...
        const total = datosMultiplicados.reduce((acc,dato)=>{
  return acc + dato
        },0)
